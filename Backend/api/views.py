...
from transformers import AutoTokenizer, AutoModelForCausalLM, T5ForConditionalGeneration, Trainer, TrainingArguments
...
def generate(request):
    try:
        ...
        try:
            ...
            return JsonResponse({"ok": False, "error": f"Model generation failed: {str(e_gen)}"}, status=500)
...
class PoisonDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if _is_t5:
            # Encode input and output separately for T5
            ...
            return {
                ...
                "labels": enc_out["input_ids"].squeeze(0),  # Trainer handles decoder_input_ids
            }
        else:
            enc = self.tokenizer(
                text_in,
...
def poison(request):
    ...
    try:
        ...
        if proc.returncode != 0:
            return JsonResponse({"ok": False, "error": proc.stderr or "Unknown error while running poisoning script"}, status=500)

        # Step 2: Load poisoned data
        poisoned_file = os.path.join(base_dir, 'Dataset', 'poisoned_dataset.json')
        if not os.path.isfile(poisoned_file):
            return JsonResponse({"ok": False, "error": f"Poisoned dataset file not found at {poisoned_file}"}, status=500)

        with open(poisoned_file, "r", encoding="utf-8") as f:
            poisoned_examples = json.load(f)

        if len(poisoned_examples) == 0:
            return JsonResponse({"ok": False, "error": "Poisoned dataset empty"}, status=500)

        # Step 3: Fine-tune model in-memory
        dataset = PoisonDataset(_tokenizer, poisoned_examples, max_length=512)
        ...
        try:
            ...
            return JsonResponse({"ok": False, "error": f"Fine-tuning failed: {str(e_train)}"}, status=500)
